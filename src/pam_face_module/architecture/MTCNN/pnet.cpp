...
std::vector<FaceBox> Pnet::final_candidate_boxes() const {
  return final_candidate_boxes_;
}
...
void Pnet::FetchData() {
  ...
  GenerateBoundingBox(conf_data, conf_size, reg_data, scale_, kPnetThreshold,
...
void Pnet::GenerateBoundingBox(const float *confidence_data,
                               int confidence_size, const float *reg_data,
                               float scale, float threshold, int feature_h,
                               int feature_w, std::vector<FaceBox> &output,
                               bool transposed) {
  int stride = 2;
  int cellSize = 12;
...
bool Pnet::Init() {
  // Generate input.
  {
    raw_input_data_.reset(new float[3 * height_ * width_],
  ...
  // Prepare outputs.
  {
    ...
    TF_Operation *score_op =
        TF_GraphOperationByName(graph_.get(), "pnet/prob1");
...
void Pnet::RunSession() {
  // run session
  TF_SessionRun(sess_.get(), NULL, run_inputs_, run_inputs_tensors_, 1,
                run_outputs_, &run_output_tensors_[0], 2, NULL, 0, NULL,
                status_.get());
