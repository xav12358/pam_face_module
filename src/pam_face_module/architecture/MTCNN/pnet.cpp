...
void Pnet::FeedML(cv::Mat const &u8x3_image) {
    cv::MatConstIterator_<cv::Vec3b> iter = u8x3_image.begin<cv::Vec3b>();
    cv::MatConstIterator_<cv::Vec3b> iter_end = u8x3_image.end<cv::Vec3b>();
...
//    nms_boxes(candidate_boxes, 0.5, NMSType::kNMS_UNION , box_list);
}
...
bool Pnet::Init() {
    // Generate input.
    {
        raw_input_data_.reset(new uint8_t[3 * height_ * width_], std::default_delete<uint8_t[]>());
        ...
        input_tensor_.reset(TF_NewTensor(TF_UINT8, raw_input_dims_.get(), 4, raw_input_data_.get(),
...
void Pnet::Process(cv::Mat const &u8x3_image) {
...
void Pnet::RunSession() {
    ...
    if (TF_GetCode(status_.get()) != TF_OK) {
        std::fprintf(stderr, "ERROR: Unable to run output_op: %s\n", TF_Message(status_.get()));
        return;
    }
}
